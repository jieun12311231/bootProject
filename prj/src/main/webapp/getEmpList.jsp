      ...
      <script>    
        ...
        //상품 선택시 단가 입력 및 체크 상태 변경
      	...
      	function makeData(){
			...
			//체크한 행만 전송할 데이터 만들기.
      		$("[name='selId']:checked").each(function(i, checkbox){  //for문돌면서 객체만드는중 
            ...
            var ord_cnt = td.eq(5).children().val();
            var ord_price = price * ord_cnt;
			...
			//ajax호출
			$.ajax({
				url:"http://localhost:81/product",  //하나의 서버안에 여러개의 프로젝트 돌리는것 가능 prj에서 boot3으로 보내기 
				method:"post",
				data: JSON.stringify(list),  //제이슨형태로 보내기 
				contentType : "application/json"  //제이슨으로 보내는거 알려주기
			})
         .then(res =>{
            console.log(res.result)
            console.log(res.size)
            console.log(res.list[0].customer)
         })
